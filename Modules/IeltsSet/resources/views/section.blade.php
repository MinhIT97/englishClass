<x-app-layout>
    ...
    <div class="set-workspace-meta">
        ...
        @endif
        <div class="glass-card set-workspace-stat">
            <strong>{{ ucfirst(str_replace('_', ' ', $sectionProgress['status'] ?? 'not_started')) }}</strong>
    ...
    @if($section->skill === 'speaking')
        <div class="glass-card set-speaking-hero">
            <div>
                <h3 style="margin-bottom: 0.75rem">Speaking Section Flow</h3>
                <p style="color: var(--text-muted); line-height: 1.7">
                    Use the prompts below to guide your answers. Open the AI speaking simulator, complete your speaking practice there, then return here and mark this section as completed so the IELTS set can track all four skills correctly.
                </p>
            </div>
            <div class="set-speaking-actions">
                <a
                    href="{{ route('student.speaking.index', ['return_to' => route('student.sets.section', [$set, $section]), 'set' => $set->title, 'section' => $section->title]) }}"
                    class="btn btn-primary"
                    style="padding: 0.85rem 1.4rem"
                >
                    Open AI Speaking Simulator
                </a>
                <form method="POST" action="{{ route('student.sets.section.complete-speaking', [$set, $section]) }}">
                    @csrf
                    <input type="hidden" name="active_seconds_delta" id="speaking-active-seconds-delta" value="0">
                    <button class="btn btn-outline" style="padding: 0.85rem 1.4rem">
                        {{ ($sectionProgress['status'] ?? null) === 'completed' ? 'Update Completion' : 'Mark Speaking Section Complete' }}
                    </button>
                </form>
            </div>
        </div>
                ...
                <div class="glass-card set-question-card">
                    <div class="set-question-card-header">
                        <div>
                            ...
                            <h3 style="margin-top: 0.85rem">Speaking Prompt {{ $loop->iteration }}</h3>
                        </div>
                    ...
                    @if(!empty($question->content['cue_card']))
                        <div class="set-speaking-note">
                            <strong>Cue card:</strong> {{ $question->content['cue_card'] }}
                        </div>
                    @endif

                    @if(!empty($question->content['follow_up']))
                        <div class="set-speaking-note">
                            <strong>Follow-up:</strong> {{ $question->content['follow_up'] }}
                        </div>
                    @endif
                </div>
            @endforeach
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const deltaInputs = Array.from(document.querySelectorAll('#active-seconds-delta, #speaking-active-seconds-delta'));
            ...
            async function syncElapsedTime() {
                ...
                try {
                    ...
                    // Keep the client timer running even if syncing fails.
                } finally {
                    syncInFlight = false;
                    renderTimer();
                }
            ...
            function captureUnsyncedDelta() {
                ...
                const deltaSeconds = Math.max(0, Math.floor((Date.now() - lastResumeAt) / 1000));
                deltaInputs.forEach((input) => {
                    input.value = String(deltaSeconds);
                });
            }
            ...
            document.querySelectorAll('form').forEach((form) => {
                form.addEventListener('submit', captureUnsyncedDelta);
            });
        });
    </script>
